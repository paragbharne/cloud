...
def getDataDiskCount():
    ...
    print "cmx.cm_server" , cmx.cm_server
    print "cmx.ssh_root_user" , cmx.ssh_root_user
    print "cmx.ssh_root_password" , cmx.ssh_root_password
...
def init_cluster():
    ...
    # if cm:
    #   print "CM was initialized properly"
    #   print "CM Object ",cm
    # else:
    #   print "CM was not initialized"
    ...
    #all_clusters=[x.name for x in api.get_all_clusters()]
...
def setup_hdfs(HA):
    ...
    cluster = api.get_cluster(cmx.cluster_name)
    service_type = "HDFS"
    #service = cluster.get_service("hdfs")   #--TSE delete later
    if cdh.get_service_type(service_type) is None:
        print "> %s" % service_type
        service_name = "hdfs"
        print "Create %s service" % service_name
    ...
    for role_type in ['DATANODE']:
        for host in management.get_hosts(include_cm_host = False):
            ...
            if host.hostId not in TSE_nodes_master and host.hostId not in TSE_nodes_edge: #--TSE
...
def setup_sqoop():
    ...
    if cdh.get_service_type(service_type) is None:
        ...
        cdh.create_service_role(service, "SQOOP_SERVER", [host for host in hosts if host.id == 1][0])  #TSE--2
...
def setup_kafka():   #TSE
    ...
    if cdh.get_service_type(service_type) is None:
        ...
        cdh.create_service_role(service, "KAFKA_SERVER", [host for host in hosts if host.id == 1][0])    #TSE--2
...
def setup_hdfs_ha():
    ...
    :return:
    """
    # api = ApiResource(cmx.cm_server, username=cmx.username, password=cmx.password, version=18)
    # cluster = api.get_cluster(cmx.cluster_name)
    try:
        print "> Setup HDFS-HA"
...
class ManagementActions:
    ...
    def __init__(self, *role_list):
        ...
        self._api = ApiResource(server_host=cmx.cm_server, username=cmx.username, password=cmx.password, version=18)
