...
for element in $HOSTENTRY
do
        line=$(echo "$element" |awk -F"," '{print $1,$2,$3}')
        echo "$line" >> /etc/hosts
done
...
# Disable the need for a tty when running sudo and allow passwordless sudo for the admin user (3 lines uncommented)
