...
if [ "$INSTALLCDH" == "True" ]
then
  if ! sh initialize-cloudera-server.sh "$CLUSTERNAME" "$key" "$mip" "$worker_ip" "$HA" "$ADMINUSER" "$PASSWORD" "$CMUSER" "$CMPASSWORD" "$EMAILADDRESS" "$BUSINESSPHONE" "$FIRSTNAME" "$LASTNAME" "$JOBROLE" "$JOBFUNCTION" "$COMPANY" "$VMSIZE" "$MYSQLUSER" "$MYSQLPASSWORD" "$MYSQLHOST">/dev/null 2>&1
  then
